...
FOR EACH ROW
BEGIN
    select  email , password from users where email = new.email or password = new.password
    IF email = new.email or password = new.password THEN
...
FOR EACH ROW
BEGIN
    -- Asigna un valor predeterminado a falso a cada registro (ningun usuario esta activo sin validación de administradores)
    select is_active from users where id_user = new.id_user;
    IF new.is_active = 1 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Los usuarios no deben ser activos aun.';
    END IF;
END;
# preuba
insert into users values (0,'hola@gmail.com','123',1,'','admin',now());
...
BEGIN
    ...
    if PASS IS NOT NULL then
        if exists(select password from users where PASS= NEW.password ) then
...
BEGIN
    ...
    declare s_tatus boolean;
    set s_tatus = new.is_active;

    select is_active into s_tatus from users;
    if s_tatus = true or 1 then
        signal sqlstate '45000' set message_text = 'Los usuarios deben ser inactivos';
    end if;
END;
drop trigger after_insert_users;
insert into users values (0,'carsi@gmail.com', '123', 0, 'url-photo','voluntario',now());
